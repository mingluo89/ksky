<!-- nav top begin -->
<header class="transparent">
    <div id="topbar" style="background-size: cover; background-repeat: no-repeat;">
        <div class="container" style="background-size: cover; background-repeat: no-repeat;">
            <div class="row" style="background-size: cover; background-repeat: no-repeat;">
                <div class="col-lg-12" style="background-size: cover; background-repeat: no-repeat;">
                    <div class="d-flex justify-content-between xs-hide" style="background-size: cover; background-repeat: no-repeat;">
                        <div class="d-flex" style="background-size: cover; background-repeat: no-repeat;">
                            <div class="topbar-widget me-5" style="background-size: cover; background-repeat: no-repeat;"><a href="/"><i class="icofont-location-pin"></i>316 Lê Văn Sỹ, Phường Tân Sơn Hòa, Thành phố Hồ Chí Minh, Việt Nam</a></div>
                            <div class="topbar-widget me-5" style="background-size: cover; background-repeat: no-repeat;"><a href="/"><i class="icofont-phone"></i>0981776666</a></div>
                            <div class="topbar-widget me-5" style="background-size: cover; background-repeat: no-repeat;"><a href="/"><i class="icofont-envelope"></i>admin@ksky.vn</a></div>
                        </div>

                        <div class="d-flex" style="background-size: cover; background-repeat: no-repeat;">
                            <div class="social-icons" style="background-size: cover; background-repeat: no-repeat;">
                                <a href="/"><i class="fa-brands fa-facebook fa-lg"></i></a>
                                <a href="/"><i class="fa-brands fa-x-twitter fa-lg"></i></a>
                                <a href="/"><i class="fa-brands fa-youtube fa-lg"></i></a>
                                <a href="/"><i class="fa-brands fa-pinterest fa-lg"></i></a>
                                <a href="/"><i class="fa-brands fa-instagram fa-lg"></i></a>
                            </div>
                        </div>
                    </div>
                </div>
            </div>
            <div class="clearfix" style="background-size: cover; background-repeat: no-repeat;"></div>
        </div>
    </div>

    <div class="container" style="background-size: cover; background-repeat: no-repeat;">
        <div class="row" style="background-size: cover; background-repeat: no-repeat;">
            <div class="col-md-12" style="background-size: cover; background-repeat: no-repeat;">
                <div class="de-flex sm-pt0" style="background-size: cover; background-repeat: no-repeat;">
                    <div class="de-flex-col" style="background-size: cover; background-repeat: no-repeat;">
                        <!-- logo begin -->
                        <div id="logo" style="background-size: cover; background-repeat: no-repeat;">
                            <a href="/">
                                <img class="logo-main" src="/demo/images/brand/logo-white.png" alt="">
                                <img class="logo-mobile" src="/demo/images/brand/logo-white.png" alt="">
                            </a>
                        </div>
                        <!-- logo end -->
                    </div>
                    <div class="de-flex-col header-col-mid" style="background-size: cover; background-repeat: no-repeat;">
                        <!-- mainemenu begin -->
                        <ul id="mainmenu">
                            <li><a class="menu-item" href="/demo/">Trang chủ</a><span></span>
                            <li><a class="menu-item" href="/demo/san-pham">Sản phẩm</a><span></span>
                            </li>
                            <li class="menu-item-has-children has-child"><a class="menu-item" href="/demo/du-an/">Dự án</a><span></span>
                                <ul>
                                    <li><a href="/demo/du-an/?name=du-an-1">Dự án 1</a></li>
                                    <li><a href="/demo/du-an/?name=du-an-2">Projects Slider</a></li>
                                    <li><a href="/demo/du-an/?name=du-an-3">Projects Carousel</a></li>
                                    <li><a href="/demo/du-an/?name=du-an-4">Project Details</a></li>
                                </ul>
                            </li>
                            <li class="menu-item-has-children has-child"><a class="menu-item" href="/demo/dich-vu">Dịch vụ</a><span></span>
                                <ul>
                                    <li><a href="/demo/dich-vu?name=dich-vu-bao-tri-may-phat-dien">Bảo trì máy phát điện</a></li>
                                    <li><a href="/demo/dich-vu?name=dich-vu-bao-tri-may-lanh">Bảo trì máy lạnh</a></li>
                                    <li><a href="/demo/dich-vu?name=dich-vu-bao-tri-he-thong-cntt">Bảo trì hệ thống CNTT</a></li>
                                    <li><a href="/demo/dich-vu?name=dich-vu-lap-dat-he-thong-camera">Hệ thống camera</a></li>
                                    <li><a href="/demo/dich-vu?name=dich-vu-cap-quang">Cáp quang</a></li>

                                    <li><a href="/demo/dich-vu?name=thiet-bi-camera-an-ninh">Thiết bị camera an ninh</a></li>
                                    <li><a href="/demo/dich-vu?name=thiet-bi-bao-dong-bao-chay">Thiết bị báo động - báo cháy</a></li>
                                    <li><a href="/demo/dich-vu?name=thiet-bi-van-phong-gia-dinh">Thiết bị Văn phòng - Gia đình</a></li>
                                    <li><a href="/demo/dich-vu?name=thiet-bi-hoi-nghi-truyen-hinh">Thiết bị hội nghị truyền hình</a></li>
                                    <li><a href="/demo/dich-vu?name=thiet-bi-vien-thong">Thiết bị viễn thông</a></li>
                                    <li><a href="/demo/dich-vu?name=thiet-bi-so-cong-nghe">Thiết bị số - Công nghệ</a></li>
                                    <li><a href="/demo/dich-vu?name=thiet-bi-dien-thong-minh">Thiết bị điện thông minh</a></li>
                                    <li><a href="/demo/dich-vu?name=thiet-bi-gia-dung">Thiết bị gia dụng</a></li>
                                    <li><a href="/demo/dich-vu?name=thiet-bi-dien-tu">Thiết bị điện tử</a></li>
                                </ul>
                            </li>
                            <li class="menu-item-has-children has-child"><a class="menu-item" href="/demo/company">Công Ty</a><span></span>
                                <ul>
                                    <li><a href="/demo/company">Về chúng tôi</a></li>
                                    <li><a href="/demo/company/#team">Đội ngũ</a></li>
                                    <li><a href="/demo/company/#partner">Khách hàng</a></li>
                                </ul>
                            </li>
                            <li><a class="menu-item" href="/demo/blog">Blog</a></li>
                            <li><a class="menu-item" href="/demo/contact">Liên hệ</a></li>
                        </ul>
                        <!-- mainmenu end -->
                    </div>
                    <div class="de-flex-col" style="background-size: cover; background-repeat: no-repeat;">
                        <div class="menu_side_area" style="background-size: cover; background-repeat: no-repeat;">
                            <a href="/demo/bao-gia" class="btn-main btn-line fx-slide hover-white" data-hover="Nhận báo giá"><span>Nhận báo giá</span></a>
                            <span id="menu-btn"></span>
                        </div>

                        <div id="btn-extra" style="background-size: cover; background-repeat: no-repeat;">
                            <span></span>
                            <span></span>
                        </div>
                    </div>
                </div>
            </div>
        </div>
    </div>
</header>
<!-- nav top end -->